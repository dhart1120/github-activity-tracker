...
class GitHubDeleteEvent(GitHubEvent):
    ...
    def summary(self):
        ...
        return f"""
        {convert_to_local(self.created_at)} {actor_name} deleted {self.ref_type} {self.ref} for repo {self.repo} {jira_tickets}
        """.strip()    

    def __str__(self):
        return f'<GitHubDeleteEvent {self.action} {self.number}>'
